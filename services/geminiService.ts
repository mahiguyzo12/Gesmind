...
const getAiClient = (): GoogleGenAI => {
  ...
  // Try to get key from process.env (injected by Vite) or import.meta.env
  let apiKey = process.env.API_KEY || "";
  
  // Fallback to GEMINI_API_KEY if API_KEY is missing (just in case)
  ...
  if (!apiKey) {
    ...
    throw new Error("Clé API Gemini manquante. Vérifiez la configuration du build ou les variables d'environnement.");
  }
  ...
  try {
      ...
      throw new Error(`Erreur initialisation Gemini: ${e.message}`);
  }
};

...
export const AIAdvisor: React.FC<AIAdvisorProps> = ({ items, currency }) => {
  ...
  const handleAnalysis = async () => {
    ...
    try {
      ...
      setError(
        "Impossible d'obtenir l'analyse IA. Vérifiez votre clé API ou réessayez plus tard."
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-6">
      <header className="mb-8">
        ...
        <p className="text-slate-500 mt-2">
          Utilisez la puissance de Gemini pour détecter les anomalies et
      ...
      {!analysis && !loading && (
        <div className="bg-white rounded-2xl p-10 text-center shadow-sm border border-slate-100">
          ...
          <h3 className="text-xl font-bold text-slate-800 mb-2">
            Prêt à analyser votre inventaire ?
          </h3>
          <p className="text-slate-500 mb-8 max-w-md mx-auto">
            Notre IA va scanner vos {items.length} produits pour identifier les
            risques de rupture, les surstocks et les opportunités d&apos;optimisation
            (Analyse en {currency.label}).
          </p>
          <button
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 border-l-4 border-l-amber-500">
            ...
            <ul className="space-y-3">
              {analysis.alerts.map((alert, idx) => (
                <li
                  key={idx}
                  className="flex items-start bg-amber-50/50 p-3 rounded-lg"
                >
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 border-l-4 border-l-green-500">
            ...
            <ul className="space-y-3">
              {analysis.suggestions.map((suggestion, idx) => (
                <li
                  key={idx}
                  className="flex items-start bg-green-50/50 p-3 rounded-lg"
                >

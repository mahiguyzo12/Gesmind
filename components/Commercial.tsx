...
export const Commercial: React.FC<CommercialProps> = ({
  ...
  const filteredProducts = accessibleInventory
    .filter(
      (item) =>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-0 md:p-2 z-50 overflow-hidden">
          <div className="bg-white rounded-none md:rounded-xl shadow-2xl w-full max-w-7xl h-full md:h-[95vh] flex flex-col animate-fade-in-up">
            {/* HEADER */}
            <div
              className={`flex justify-between items-center px-3 py-2 border-b shrink-0 ${modalMode === 'PURCHASE' ? 'bg-blue-50 border-blue-100' : 'bg-slate-50 border-slate-100'}`}
            >
              <div className="flex items-center gap-2">
                <h3
                  className={`text-base font-bold flex items-center ${modalMode === 'PURCHASE' ? 'text-blue-700' : 'text-slate-800'}`}
                >
                  {modalMode === 'SALE' ? (
              ...
              <button
                ...
                className="text-slate-400 hover:text-red-500 bg-white p-1 rounded-full hover:bg-red-50 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {modalMode === 'PURCHASE' && (
              <div className="bg-blue-100 border-b border-blue-200 px-4 py-2 text-xs text-blue-800 flex items-center shrink-0">
                ...
                <span>
                  Cette facture sera comptabilisée dans l&apos;activité globale
                  (Chiffre d&apos;Affaires/Volume), mais{' '}
                  <strong>
                    n&apos;impactera la caisse que si vous saisissez un &quot;Montant
                    Versé&quot; ci-dessous
                  </strong>
                  .
                </span>
              ...
              <div className="bg-white p-2 border-b border-slate-200 shrink-0 shadow-sm z-20">
                <div className="flex flex-col md:flex-row gap-2">
                  ...
                  <div className="w-full md:w-48">
                    ...
                    >
                      <option value="">
                        -- {modalMode === 'SALE' ? 'Client' : 'Fournisseur'} --
                      </option>
                      {modalMode === 'SALE'
                        ? customers.map((c) => (
                  ...
                  <div
                    className="flex-1 flex gap-1 relative"
                  ...
                  >
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        onFocus={() => {
                          if (productSearchTerm) setIsProductDropdownOpen(true);
                        }}
                        autoComplete="off"
                      />
                            ...
                            filteredProducts.map((item, index) => (
                              ...
                              >
                                ...
                                <div className="text-right ml-2">
                                  ...
                                  <span
                                    className={`text-[10px] ${item.quantity > 0 ? 'text-emerald-600' : 'text-red-500'}`}
                                  >
                                    Stock: {item.quantity}
                                  </span>
                                </div>
                    ...
                    <input
                      ...
                      value={qtyInput}
                      onChange={(e) =>
                        setQtyInput(parseInt(e.target.value) || 0)
                      }
                      placeholder="Qté"
                ...
                <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-start px-2">
                  <div className="flex flex-col">
                    <span className="text-[10px] uppercase font-bold text-slate-500">
                      Total Facture (Activité)
                    </span>
                    <span
                      className={`text-xl md:text-2xl font-bold ${modalMode === 'PURCHASE' ? 'text-blue-700' : 'text-indigo-700'}`}
                    >
                ...
                <div className="flex items-center gap-2 w-full md:w-auto bg-white p-1.5 rounded-xl border border-slate-200 shadow-sm">
                  <div className="flex items-center gap-2 px-2 border-r border-slate-100 pr-3">
                    ...
                    <div className="flex flex-col items-end">
                      ...
                      <span
                        className={`text-sm font-mono font-bold ${modalBalanceDue > 0.01 ? 'text-red-600' : 'text-emerald-600'}`}
                      >

...
export const Users: React.FC<UsersProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (newRole === 'ADMIN') {
      if (!finalPermissions.includes('users.view'))
        finalPermissions.push('users.view');
      if (!finalPermissions.includes('users.manage'))
        finalPermissions.push('users.manage');
      if (!finalPermissions.includes('settings.view'))
        finalPermissions.push('settings.view');
    }
    ...
    const userData: Partial<User> = {
      ...
      avatar:
        newAvatar ||
        `https://ui-avatars.com/api/?name=${encodeURIComponent(newName)}&background=random&color=fff`,
    };
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">
            Créez des comptes et définissez les permissions détaillées.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {users.map((user) => {
          ...
          return (
            <div
              key={user.id}
              className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between h-full group relative"
            >
              <div className="absolute top-4 right-4 flex items-center">
                <div
                  className={`w-2 h-2 rounded-full mr-1.5 ${status.isOnline ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`}
                ></div>
                <span
                  className={`text-[10px] font-medium ${status.isOnline ? 'text-emerald-600' : 'text-slate-400'}`}
                >
                  ...
                  {newRole === 'SELLER' && (
                    <div>
                      ...
                      <input
                        ...
                        value={newCommission}
                        onChange={(e) =>
                          setNewCommission(parseFloat(e.target.value))
                        }
                      />
                    </div>
                  )}
                ...
                <div className="lg:col-span-2 bg-slate-50 p-6 rounded-xl border border-slate-100">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[60vh] overflow-y-auto pr-2">
                    {PERMISSION_CATEGORIES.map((category) => {
                      const catKeys = category.actions.map(
                        (a) => `${category.id}.${a.id}`
                      );

...
export const SalesTeam: React.FC<SalesTeamProps> = ({
  users,
  transactions,
  currency,
}) => {
  ...
  const getUserStats = (userId: string, commissionRate: number = 0) => {
    const userSales = transactions.filter(
      (t) => t.sellerId === userId && t.type === 'SALE'
    );
  ...
  const selectedSeller = selectedSellerId
    ? users.find((u) => u.id === selectedSellerId)
    : null;
  const selectedSellerStats = selectedSeller
    ? getUserStats(selectedSeller.id, selectedSeller.commissionRate)
    : null;
    ...
    <div className="space-y-6">
      <header className="mb-6">
        ...
        <p className="text-slate-500 mt-2">
          Suivi des performances individuelles et des factures associées.
        </p>
      </header>
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      className={`font-bold truncate ${isSelected ? 'text-white' : 'text-slate-800'}`}
                    >
                      {seller.name}
                    </p>
                    <div className="flex items-center text-xs mt-1 space-x-2">
                      <span
                        className={`${isSelected ? 'text-indigo-100' : 'text-slate-500'}`}
                      >
                        {stats.count} Ventes
                      </span>
                      <span
                        className={`px-1.5 py-0.5 rounded ${isSelected ? 'bg-indigo-500 text-white' : 'bg-slate-100 text-slate-600'}`}
                      >
                  ...
                  <div className="text-right">
                    ...
                    <ChevronRight
                      className={`w-4 h-4 ml-auto mt-1 ${isSelected ? 'text-indigo-200' : 'text-slate-300'}`}
                    />
                  </div>
                </div>
              </button>
        ...
        <div className="lg:col-span-2">
          {selectedSeller && selectedSellerStats ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
                  ...
                  <p className="text-2xl font-bold text-slate-800">
                    {formatCurrency(selectedSellerStats.total)}
                  </p>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
                  ...
                  <p className="text-2xl font-bold text-indigo-600">
                    {formatCurrency(selectedSellerStats.commission)}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
                <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                  ...
                  <span className="text-xs text-slate-500">
                    {selectedSellerStats.count} factures
                  </span>
                </div>
            ...
            <div className="h-full flex flex-col items-center justify-center bg-white rounded-2xl border border-slate-100 border-dashed p-12 text-slate-400">
              ...
              <p className="text-lg">
                Sélectionnez un commercial pour voir les détails.
              </p>
            </div>
          )}
        </div>

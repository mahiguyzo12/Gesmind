            ...
            <form onSubmit={handleSaveStore} className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 space-y-6 animate-fade-in">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4 border-t border-slate-100 dark:border-slate-700 pt-4">
                  ...
                  {isAdmin && storeId && (
                    <div className="bg-slate-100 dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-600 flex items-center justify-between">
                        <div>
                            <span className="text-xs font-bold text-slate-500 uppercase block">ID Store (Admin Only)</span>
                            <code className="text-sm font-mono text-slate-700 dark:text-slate-300 select-all">{storeId}</code>
                        </div>
                        <button 
                            type="button"
                            onClick={() => {navigator.clipboard.writeText(storeId); alert("ID copié !")}}

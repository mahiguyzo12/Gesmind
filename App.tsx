    ...
    if (currentUser && currentStoreId && isDbConfigured) {
        const runAutoCloseCheck = async () => {
            try {
                ...
                console.warn("Auto-close check failed silently:", e);
            }
        };
        // Increase delay to ensure Auth is fully propagated
        const timer = setTimeout(runAutoCloseCheck, 5000);
        return () => clearTimeout(timer);
    }
  }, [currentUser, currentStoreId, isDbConfigured]);

  // --- GLOBAL UPDATE CHECK (Run once on mount) ---
  useEffect(() => {
    const check = async () => {
       // Utilise le dépôt par défaut pour vérifier les mises à jour globales de l'application
       // Ceci permet d'avoir la notif AVANT de se connecter à une boutique
       const repoToCheck = DEFAULT_SETTINGS.githubRepo || 'mahiguyzo12/Gesmind';
       try {
           const update = await checkForUpdates(repoToCheck);
           if (update.hasUpdate) {
               console.log("Mise à jour disponible :", update.latestVersion);
               setAvailableUpdate(update);
           }
       } catch (e) { 
           console.error("Global update check failed", e); 
       }
    };
    
    // Délai pour laisser l'UI s'initialiser
  ...
  if (currentView === ViewState.MENU) {
    return (
      <>
        ...
        />
        {/* Banner visible dans le menu principal */}
        {availableUpdate && <UpdateBanner update={availableUpdate} onClose={() => setAvailableUpdate(null)} themeColor={storeSettings.themeColor || '#4f46e5'} />}
        {isLogoutModalOpen && (

...
const authGuard = (startSnapshot: () => () => void) => {
    ...
    ensureAuthReady().then(() => {
        if (!cancelled && db) { // Guard db existence
            unsub = startSnapshot();
        }
    });
    return () => {
...
const subscribeToCollection = (storeId: string, collectionName: string, callback: (data: any[]) => void, orderField: string = "name", orderDir: "asc" | "desc" = "asc") => {
    ...
    return authGuard(() => {
        ...
        const q = query(collection(db, "stores", storeId, collectionName), orderBy(orderField, orderDir));
...
export const performCashClosing = async (
    ...
    try {
        ...
        const closingRef = doc(dbInstance, "stores", storeId, "cashClosings", closingData.id);
    ...
    try {
        ...
        for (const dateKey of datesToClose) {
            ...
            if (closingSnap.exists()) {
                ...
                let totalSales = 0, amountCash = 0, amountMobile = 0, amountCard = 0;
                dayTxs.forEach(tx => {

...
export const processForgottenClosings = async (
    ...
    if (!auth?.currentUser) {
        console.warn("processForgottenClosings: User not authenticated. Skipping.");
        return 0;
    }
    ...
    try {
        // Query transactions for this register
        ...
        txSnapshot.forEach(docSnap => {
            ...
            // Filter for past dates that are not locked
        ...
        for (const dateKey of datesToClose) {
            ...
            // Create a new batch for this date
            const batch = writeBatch(db);
            ...
            if (!closingSnap.exists()) {
                ...
                const autoClosing: CashClosing = {
                    ...
                    status: 'closed',
                    autoClosed: true,
                    comment: `Clôture automatique (Oubli détecté) - ${userName || 'Système'}`,
            ...
            // Lock all transactions for this day
            dayTxs.forEach(tx => {
        ...
        if (e.code === 'permission-denied') {
             console.warn("Permission denied in processForgottenClosings. Check Firestore rules.");
        }
        return 0;
    }
};
